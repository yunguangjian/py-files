...
async def download(url,name):
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as c_resp:

            nc=str(re.findall(r'<div id="chaptercontent" class="Readarea ReadAjax_content">(.*)',await c_resp.text())[0]).replace(r'<br /><br />','\n')
...
async def main():
    ...
    for zj_url,zj_bt in zj_jq:
        ...
        tasks.append(download(url,name))
